...
_DEBUG_TEMB_PATCHED = False

def _maybe_patch_temb_debug():
    global _DEBUG_TEMB_PATCHED
    if _DEBUG_TEMB_PATCHED:
        return
    if os.environ.get("LINGBOT_VA_DEBUG_TEMB", "0") != "1":
        return

    orig_forward = model_module.WanTransformerBlock.forward

    def debug_forward(self, hidden_states, encoder_hidden_states, temb, rotary_emb, self_attn_mask=None, cross_attn_mask=None, update_cache=0, cache_name='pos'):
        rank = int(os.environ.get('RANK', '-1'))
        if not hasattr(self, '_debug_block_id'):
            self._debug_block_id = id(self)
        try:
            temb_f = temb.detach().float()
            sst_f = self.scale_shift_table.detach().float()
            logger.info(
                f"temb_debug_pre rank={rank} block_id={self._debug_block_id} hidden_shape={tuple(hidden_states.shape)} hidden_dtype={hidden_states.dtype} hidden_device={hidden_states.device} hidden_stride={hidden_states.stride()} "
                f"temb_shape={tuple(temb.shape)} temb_dtype={temb.dtype} temb_device={temb.device} temb_stride={temb.stride()} temb_min={temb_f.min().item():.4f} temb_max={temb_f.max().item():.4f} temb_finite={bool(torch.isfinite(temb_f).all().item())} "
                f"sst_shape={tuple(self.scale_shift_table.shape)} sst_dtype={self.scale_shift_table.dtype} sst_device={self.scale_shift_table.device} sst_stride={self.scale_shift_table.stride()} sst_min={sst_f.min().item():.4f} sst_max={sst_f.max().item():.4f} sst_finite={bool(torch.isfinite(sst_f).all().item())}"
            )
            out = orig_forward(self, hidden_states, encoder_hidden_states, temb, rotary_emb, self_attn_mask=self_attn_mask, cross_attn_mask=cross_attn_mask, update_cache=update_cache, cache_name=cache_name)
            return out
        except Exception as exc:
            temb_f = temb.detach().float()
            sst_f = self.scale_shift_table.detach().float()
            logger.error(
                f"temb_debug_fail rank={rank} block_id={self._debug_block_id} hidden_shape={tuple(hidden_states.shape)} hidden_dtype={hidden_states.dtype} hidden_device={hidden_states.device} hidden_stride={hidden_states.stride()} "
                f"temb_shape={tuple(temb.shape)} temb_dtype={temb.dtype} temb_device={temb.device} temb_stride={temb.stride()} temb_min={temb_f.min().item():.4f} temb_max={temb_f.max().item():.4f} temb_finite={bool(torch.isfinite(temb_f).all().item())} "
                f"sst_shape={tuple(self.scale_shift_table.shape)} sst_dtype={self.scale_shift_table.dtype} sst_device={self.scale_shift_table.device} sst_stride={self.scale_shift_table.stride()} sst_min={sst_f.min().item():.4f} sst_max={sst_f.max().item():.4f} sst_finite={bool(torch.isfinite(sst_f).all().item())} "
    ...
    model_module.WanTransformerBlock.forward = debug_forward
    _DEBUG_TEMB_PATCHED = True
...
class Trainer:
    def __init__(self, config):
        ...
        disable_fsdp = os.environ.get("LINGBOT_VA_DISABLE_FSDP", "0") == "1"
        if disable_fsdp:
            logger.info("Setting up DDP (FSDP disabled)...")
            self.transformer = self.transformer.to(device=self.device, dtype=self.dtype)
            self.transformer = torch.nn.parallel.DistributedDataParallel(
                self.transformer,

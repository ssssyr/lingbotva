...
job_name="${JOB_4GPU:-ur10-gamepad-xyzgripper-relative-10hz-max256-fsdp-pm1-r3}"
watch_log_dir="${repo_root}/logs/train_watch"
watch_log_file="${WATCH_LOG_FILE:-${watch_log_dir}/real_ur10_watcher.log}"
watch_pid_file="${WATCH_PID_FILE:-${watch_log_dir}/real_ur10_watcher.pid}"
watch_tmux_session="${WATCH_TMUX_SESSION:-real-ur10-watcher}"
python_bin="${WATCH_PYTHON:-python}"

mkdir -p "${watch_log_dir}"

export WATCH_TITLE="${WATCH_TITLE:-Real UR10 Watcher}"
